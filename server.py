...
def registration(login, password, name):
    cur = con.cursor()
    cur.execute("SELECT count() FROM 'user' WHEN login = %(login)s",
                {"login": login})
    rows = cur.fetchall()
    if rows != 0:
        rows = "Этот логин уже кто-то использует"
    else:
        cur.execute("INSERT INTO 'user' VALUES %(login)s, %(password)s, %(name)s, заказчик)",
...
def addition_fittings(id_fittings, name, image, width, price):
    cur = con.cursor()
    cur.execute("SELECT count() FROM Fittings WHEN id_fittings = %(id_fittings)s",
...
def addition_cloth(id_cloth, name, colour, picture, image, composition, length, price):
    cur = con.cursor()
    cur.execute("SELECT count() FROM cloth WHEN id_cloth = %(id_cloth)s",
                {"id_cloth": id_cloth})
    rows = cur.fetchall()
    if rows != 0:
...
def order_list(login):
    cur = con.cursor()
    cur.execute(
        "SELECT picture, name, date, quantity FROM 'order' JOIN Ordered_products ON ('order'.id_order = Ordered_products.id_order) JOIN Product ON (Ordered_products.id_product = Product.id_product) JOIN 'user' ON (customer = login) WHEN login = %(login)s",
        {"login": login})
    rows = cur.fetchall()
    return (json.dumps(rows, ensure_ascii=False)).encode('UTF-16')
...
def adding_order(date, customer, id_product):
    ...
    rows1 = cur.fetchall()
    cur.execute("SELECT price FROM product WHEN id_product = %(id_product)s",
